...
class LoginSerializer(serializers.ModelSerializer):
    ...
    def validate(self, attrs):
        ...
        if not user:
            raise AuthenticationFailed('Hisob maʼlumotlari notoʻgʻri, qayta urinib koʻring')

        if not user.is_active:
            raise AuthenticationFailed('Hisob o‘chirilgan, administrator bilan bog‘laning')
        if not user.is_verified:
            raise AuthenticationFailed('Elektron pochta tasdiqlanmagan')
